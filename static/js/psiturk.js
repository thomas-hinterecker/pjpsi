 ...
 * API *
 ******/
var PsiTurk = function(uniqueId, adServerLoc, mode) { // adServerLoc
	...
	var TaskData = Backbone.Model.extend({
		urlRoot: adServerLoc + "/sync", // Save will PUT to /sync, with mimetype 'application/JSON'
		id: uniqueId,
	...
	******************************************************/
	...
	self.preloadImages = function(imagenames) {
		$(imagenames).each(function() {
			image = new Image();
			image.src = adServerLoc + '/static/' + this;
		});
	};
	...
	self.preloadPages = function(pagenames) {
		...
		$(pagenames).each(function() {
			$.ajax({
				url: adServerLoc + '/templates/' + this,
	...
	self.getPage = function(pagename) {
		pagename = adServerLoc + '/templates/' + pagename;
	...
	self.complete = function() {
		...
		window.location.href = self.taskdata.adServerLoc + '/complete/' + self.taskdata.id;
	}
